...
def Binloss(y_pred, batch_size, device, is_transformer=0):
    ...
    targets = torch.tensor([1] * batch_size + [0] * batch_size, dtype = torch.float32).to(y_pred.device)
    ...
    loss = F.binary_cross_entropy(torch.cat([y_anomaly_max, y_normal_max], dim  = 0), targets)
